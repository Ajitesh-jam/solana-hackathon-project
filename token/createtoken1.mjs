...
// APPROACH 1: PROGRAM-CONTROLLED MINTING
// Create a custom program that has minting authority
...
// 1. Create token as before
const { rpc, rpcSubscriptions, sendAndConfirmTransaction } = createSolanaClient({
  urlOrMoniker: "devnet"
});

const secretKey = Uint8Array.from(process.env.PVT_KEY);
const base58SecretKey = bs58.encode(secretKey);
const signer = await createKeyPairSignerFromBytes(bs58.decode(base58SecretKey));

console.log("Token creator address:", signer.address);
...
// 2. IMPORTANT CHANGE: Create a Program Derived Address (PDA) that will have mint authority
// This is a placeholder - in reality, you would calculate this from your program's ID
...
// 3. Create the token but set the PDA as mint authority
const transaction = createTransaction({
  feePayer: signer,
  version: "legacy",
  instructions: [
    ...
    getInitializeMintInstruction(
      {
        mint: mint.address,
        mintAuthority: mintAuthorityPDA.toBase58(), // PDA as mint authority
        freezeAuthority: signer.address, // You retain freeze authority
        decimals: 9
      },
      { programAddress: tokenProgram }
    ),
    getCreateMetadataAccountV3Instruction({
      ...
      mint: mint.address,
      metadata: metadataAddress,
      mintAuthority: signer, // You're still signing this transaction
      payer: signer,
      data: {
        sellerFeeBasisPoints: 0,
        collection: null,
...
console.log(`Token created: ${mint.address}`);
console.log(`This token can be minted by anyone through your program at: ${PROGRAM_ID}`);